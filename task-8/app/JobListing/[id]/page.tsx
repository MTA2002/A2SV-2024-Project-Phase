...
const Skills = ({ text }: { text: string }) => {
  return (
    <div className="bg-[#F8F8FD] p-2 text-[#40329a]">
      <p>{text}</p>
    </div>
  );
};
  ...
  useEffect(() => {
    fetch(`https://akil-backend.onrender.com/opportunities/${id}`)
      ...
      .then((data) => {
        ...
        console.log(jobData.categories[0].split(" "));
        setJob(jobData);
      });
  }, [id]);
      ...
      <div className="w-9/12 flex flex-col gap-12">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 pl-2">
            {job.responsibilities.split(".").map((responsibiliy, index) => {
              return (
                responsibiliy && (
      ...
      <div className="w-3/12 flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex gap-2">
            ...
            >
              {job.categories[0].split(" ")[0]}
            </button>
            {job.categories.length > 1 && (
              <Button
                content={job.categories[1].split(" ")[0]}
                bgColor="#56CDAD1A"
                textColor="#56CDAD"
              />
            )}
